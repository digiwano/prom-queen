...
const isArrayIsh = thingy =>
  Symbol.iterator in thingy ||
  ("length" in thingy &&
    typeof thingy.length === "number" &&
    "0" in thingy &&
    (thingy.length === 0 || thingy.length - 1 in thingy));
const isObject = thingy => thingy instanceof Object && Object.prototype.toString() === thingy.toString();

const fromPromise = (thingy, func) => thingy.then(res => listish(res, func));
const fromArray = (thingy, func) => Promise.resolve(func(thingy));
const fromArrayIsh = (thingy, func) => Promise.resolve(func(Array.from(thingy)));
const fromObject = (thingy, func) => {
  ...
  return Promise.resolve(func(values)).then(results => results.reduce(reducer, {}));
};

const transforms = [[isArray, fromArray], [isArrayIsh, fromArrayIsh], [isPromise, fromPromise], [isObject, fromObject]];
...
function complain(thingy, action) {
  ...
  const complaint =
    "listish expects arguments (thingy, func) " + `but received (${whateverThisThingIs}, ${iDontEvenWantToKnow})`;
